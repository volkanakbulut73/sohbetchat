...
const LandingPage: React.FC<LandingPageProps> = ({ onEnter, onRegisterClick, onAdminClick }) => {
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      ...
      // Check if user exists in registrations via storageService
      const user = await storageService.loginWithGoogle(email);
      
      if (user) {
        onEnter(user);
      } else {
        // First time Google login - Create PENDING registration
      ...
      // Catch "pending" error from loginWithGoogle
      if (err.message && err.message.includes('onaylanmadı')) {
  ...
  const handleGoogleLogin = () => {
    if ((window as any).google) {
      ...
      alert("Google servislerine şu an ulaşılamıyor. Lütfen sayfayı yenileyin.");
    }
  };
    ...
    <div className="min-h-screen bg-[#0b0f14] text-gray-300 font-mono flex flex-col selection:bg-[#00ff99] selection:text-black selection:font-bold">
      ...
      <section className="py-24 bg-[#0b0f14] border-t border-gray-900 flex flex-col items-center">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
          ...
          {/* Card 2 */}
          <div className="bg-[#121820] border border-gray-800 p-8 space-y-8 hover:border-[#00ff99]/50 transition-all shadow-[inset_0_0_40px_rgba(0,0,0,0.5)]">
            <div className="w-16 h-16 bg-gray-900 border border-gray-800 flex items-center justify-center text-[#00ff99]">
              <FileText size={32} />
            </div>
            <div className="space-y-6">
              <h3 className="text-xl font-black text-white uppercase tracking-widest italic">SABIKA KAYDI KONTROLÜ</h3>
              <ul className="space-y-4">
                <li className="flex items-center gap-3 text-xs font-bold text-gray-400">
          ...
          <div className="bg-[#121820] border border-gray-800 p-8 space-y-8 hover:border-[#00ff99]/50 transition-all">
            ...
            <div className="space-y-6">
              <h3 className="text-xl font-black text-white uppercase tracking-widest italic">ÇALIŞAN OLMA ZORUNLULUĞU</h3>
              <ul className="space-y-4">
                <li className="flex items-center gap-3 text-xs font-bold text-gray-400">
      ...
      <section className="py-24 bg-[#0b0f14] px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-6 pt-4">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-12">
                 <div className="flex items-center gap-4 group">
                    <ChevronRight size={16} className="text-[#00ff99] group-hover:translate-x-1 transition-transform" />
                    <span className="text-[13px] font-bold text-gray-300">“Naber millet?” samimiyeti</span>
                 </div>
                 <div className="flex items-center gap-4 group">
                    <ChevronRight size={16} className="text-[#00ff99] group-hover:translate-x-1 transition-transform" />
                    <span className="text-[13px] font-bold text-gray-300">Hakaret, taciz, spam yok</span>
                 </div>
                 <div className="flex items-center gap-4 group">
                    <ChevronRight size={16} className="text-[#00ff99] group-hover:translate-x-1 transition-transform" />
                    <span className="text-[13px] font-bold text-gray-300">Geyik serbest, saygı şart</span>
                 </div>
                 <div className="flex items-center gap-4 group">
                    <ChevronRight size={16} className="text-[#00ff99] group-hover:translate-x-1 transition-transform" />
                    <span className="text-[13px] font-bold text-gray-300">Moderasyon her an aktif</span>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="bg-[#0b0f14] border-2 border-white/20 shadow-2xl rounded-sm overflow-hidden transform rotate-1">
            <div className="bg-[#121820] px-4 py-2 border-b border-white/10 flex items-center justify-between">
              ...
              <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Status: connected to Workigom (irc.workigomchat.online)</div>
            ...
            <div className="p-6 font-mono text-[13px] space-y-3 min-h-[350px]">
              ...
              <p className="text-gray-500 italic">Kanal girişi bekleniyor...</p>
      ...
      <section className="py-32 bg-[#0b0f14] relative overflow-hidden flex flex-col items-center justify-center text-center">
        ...
        <div className="relative z-10 space-y-12">
          ...
          <p className="text-gray-500 font-black tracking-[0.4em] text-xs uppercase">W O R K I G O M C H A T . O N L I N E</p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <button 
              onClick={onRegisterClick}
              className="bg-[#00ff99] text-black px-12 py-5 text-sm font-black flex items-center gap-3 hover:bg-white transition-all"
            >
          ...
          <div className="bg-[#121820] border-2 border-gray-800 w-full max-w-md p-8 shadow-2xl relative">
            ...
            <div className="mt-8 pt-6 border-t border-gray-800 text-center">
              ...
              <button 
                ...
                className="w-full bg-white text-black py-3 text-xs font-black uppercase hover:bg-gray-200 transition-all flex items-center justify-center gap-3 border border-gray-800"
              >
